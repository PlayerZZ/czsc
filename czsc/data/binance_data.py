...
class BinanceData:
    ...
    def handle_socket_message_5m(self, msg):
        # 组装成RawBar
        bar = RawBar(symbol=self.symbol, dt=pd.to_datetime(msg['k']['t'], unit='ms'),
                     id=msg['k']['i'], freq=Freq.F5, open=float(msg['k']['o']),
                     close=float(msg['k']['c']), high=float(msg['k']['h']),
                     low=float(msg['k']['l']), vol=float(msg['k']['v']),)

        # 获取最后一个k线
        last_bar = self.klines_5m_RawBar[-1]
        # 如果最后一个k线的时间小于当前k线的时间，说明是新的一根k线
        if last_bar.dt < bar.dt:
            self.klines_5m_RawBar.append(bar)
        # 如果最后一个k线的时间等于当前k线的时间，说明是最后一根k线的更新
        elif last_bar.dt == bar.dt:
            self.klines_5m_RawBar[-1] = bar
        else:
            print("error happened")
    def handle_socket_message_15m(self, msg):
        # 组装成RawBar
        bar = RawBar(symbol=self.symbol, dt=pd.to_datetime(msg['k']['t'], unit='ms'),
                     id=msg['k']['i'], freq=Freq.F15, open=float(msg['k']['o']),
                     close=float(msg['k']['c']), high=float(msg['k']['h']),
                     low=float(msg['k']['l']), vol=float(msg['k']['v']),)

        # 获取最后一个k线
        last_bar = self.klines_15m_RawBar[-1]
        # 如果最后一个k线的时间小于当前k线的时间，说明是新的一根k线
        if last_bar.dt < bar.dt:
            self.klines_15m_RawBar.append(bar)
        # 如果最后一个k线的时间等于当前k线的时间，说明是最后一根k线的更新
        elif last_bar.dt == bar.dt:
            self.klines_15m_RawBar[-1] = bar
        else:
            print("error happened")
